...
const rskurl = 'http://192.168.17.34:8080/banking-console/aSelectOptBpCodeProcess.json';
let treenodeidpolyFill;
const toggle = new Array(10000);
            ...
            editoptions: {
                ...
                dataEvents: [{
                    ...
                    fn(e) {
                        ...
                        const ele = e;
                        const selText = $(`#${e.target.id}>option:selected`).text();
                        const temp = selText.split('(');
                        const seltype = +temp[1][0];
                        // self.currentType = seltype;
                        const template = '';
                        let readonly = '';
                        let lastS = '';

                        const prefirst = '<input value="" type="text" role="textbox" class="';
                        let premiddle = '';
                        if (seltype == 3) {
                        ...
                        const prelast = '" style="width: 96%;" ';
                        const eleId = `id="${$(e.target).attr('id')}_input" `;
                        const eleName = `name="${self.epcRowId}_` + 'paraV"' + ' ';
                        const eleRowId = `rowid="${$(e.target).attr('rowid')}" `;
                        const last = '/>';
                        ...
                        const inputtemplate = prefirst + premiddle + prelast + readonly + eleId + eleName + eleRowId + lastS + last;
        ...
        onSelectRow(id, status, e) {
            ...
            if (e) {
                ...
                if ($(e.target).attr('class') == 'glyphicon glyphicon-edit') {
                    ...
                    const ELE = $($(e.target).parent().parent().parent().parent()).children()[0];
                    const ELE1 = $($(e.target).parent().parent().parent().parent()).children()[1];
                    // console.log(ELE1)
                    const currentParName = $(ELE).attr('title').split('-')[0];
                    ...
                    const ELETitle = currentParName.split('-')[0];
                    const ELEId = $($(ELE).children()[0]).attr('id');
                    $(`#${ELEId}`).trigger('click');
                    // console.log(self.currentType)
                    // console.log("编辑事件执行")
                    if (clickeditseltype != 0 && clickeditseltype != 3) {
                        for (let k = 0; k < self.whichisorarechecked.length; k++) {
                ...
                if ($(e.target).attr('class') == 'glyphicon glyphicon-save') {
                    ...
                    const data = $('#jqGrid1').jqGrid('getRowData');
                    let temp = {};
                    const tempA = [];
...
Evtcf.prototype.paramSelect = function() {
    let temp = '';
    const length = self.rspar.length;

    for (item of self.rspar) {
        let ipttype = '';
        const number = item.inputtype;
...
Evtcf.prototype.saveRspa = function() {
    const test = ['name', 'chnname', 'inputtype'];
    const ELE = $('.rlpa');
    const par = {};
    const self = this;
...
function beforeClick(treeId, treeNode) {
    // console.log(treeId)
    const zTree = $.fn.zTree.getZTreeObj(`treeDemo${treenodeidpolyFill}`);
...
function onCheck(e, treeId, treeNode) {
    ...
    let zTree = $.fn.zTree.getZTreeObj(`treeDemo${treenodeidpolyFill}`),
    ...
    const cityObj = $(`.citySel${treenodeidpolyFill}`);
...
function showMenu(c) {
    // console.log("啦啦啦" + idE)
    const cityObj = $(`.citySel${treenodeidpolyFill}`);
    const cityOffset = $(`.citySel${treenodeidpolyFill}`).offset();
    $(`.menuContent${c}`).css({
